...
### D-30: Sweep re-run attempted for real — Python was never the actual blocker; this environment's network policy is — **BLOCKED, not run, nothing fabricated**

**Date:** 2026-09-08, in the environment that inherited D-29's reconciliation branch. Goal: replace TASKS.md's `[~]` on the D-28 profile sweep with a genuine `[x]` by actually running `src/demo/run_profiles_demo.py` and committing the real `artifacts/demo_profiles_endtoend.txt`. It did not get far enough to run the sweep at all — the blocker was one stage upstream of what D-29 diagnosed, and this entry exists to say exactly that rather than let D-29's diagnosis stand unchecked.

**Step 1 — Python version, re-verified rather than assumed carried over.** D-29 reported this container running 3.11.15 against a 3.13 pin. That was **this session's environment, not a fixed fact** — checked fresh here rather than trusted from the earlier entry. Result: `/usr/bin/python3.13` (3.13.12) is present alongside 3.11 and 3.12. A fresh venv on 3.13 installed `requirements.txt` clean — `numpy 2.4.1`, `pandas 2.3.3`, `zarr 3.3.0`, `xarray 2026.7.0`, `imdlib 0.1.21`, `xgboost 3.2.0`, `scikit-learn 1.8.0`, `twilio 9.11.0` all resolved and imported. **One gap in `requirements.txt` itself, noted but not fixed here** (out of this task's scope): `src/data/imd_bulletins.py:144` does `import pypdf` and it is not in `requirements.txt` at all, pinned or not — installed unpinned (`pypdf 6.18.0`) to proceed. So: Python 3.13 was never actually the blocker in this environment; it just hadn't been checked here yet.

**Step 2 — no cached data exists, anywhere.** `data/` does not exist in this container (gitignored by design, per `CLAUDE.md`). Searched the whole filesystem for `bulletin_texts.json` or any IMD/imdlib cache directory from an earlier session: nothing. Checked every remote branch (`main`, both `claude/*` branches) for an accidentally-committed `data/` path: nothing, as expected. This is confirmed to be a genuinely fresh environment, not one carrying forward artifacts D-29's environment had.

**Step 3 — real acquisition attempted, and blocked at the network, not the code.** `src/demo/case.py::build_case()` needs `load_mcz_rainfall()` (IMD 0.25° gridded rainfall via `imdlib`, from `imdpune.gov.in`) and, through `build_seasonal_table()`, the teleconnection indices in `src/data/indices.py` (Niño3.4 from NOAA CPC, DMI from NOAA PSL, RMM from Australian BOM). Per `CLAUDE.md` and the modules' own docstrings, these are the *only* permitted acquisition paths — no substitute source, no constructed filename. Ran them for real, exactly as written, no code changes:

  - `requests.get()` to all four hosts (`www.cpc.ncep.noaa.gov`, `psl.noaa.gov`, `www.bom.gov.au`, `imdpune.gov.in`) failed identically: `ProxyError` / `Tunnel connection failed: 403 Forbidden`.
  - Also tried `internal.imd.gov.in` (the Extended Range bulletin archive listing `src/data/imd_bulletins.py` reads) — same failure.
  - Cross-checked against the proxy's own diagnostic endpoint (`$HTTPS_PROXY/__agentproxy/status`) rather than trusting the Python traceback alone: `recentRelayFailures` logged all five hosts as `connect_rejected` / `"gateway answered 403 to CONNECT (policy denial or upstream failure)"`, each at the exact moment of the attempt above.
  - `pypi.org` (needed for the Step 1 install, and separately confirmed) returns `200` through the same proxy — so this is a **targeted policy denial of these specific external hosts**, not a general network fault. The environment's outbound policy allowlists package registries and nothing else these five hosts belong to.

**Nothing was worked around.** Two tempting substitutes were considered and rejected, both for the same reason:
  1. Constructing or guessing the data instead of fetching it — explicitly the failure mode `src/data/imd_bulletins.py`'s own docstring documents three real past instances of (D-15), and exactly what CLAUDE.md Rule 1 exists to stop.
  2. Using the `WebFetch` tool (which runs outside this container's network policy) to pull the same numbers via an AI-summarized markdown transcription — rejected because `imdlib` needs raw binary `.GRD` files WebFetch cannot fetch at all, and because feeding a summarizing model's transcription of a numeric time series into a decision engine is a transcription-risk substitute for real data, not real data. Silently swapping one for the other would look identical in the output to the real thing and would be exactly as fabricated as writing the numbers from memory.
