...
- [x] Confirmed end-to-end for all four profiles with no manual intervention; `run_profiles_demo --repeat 5` gives an identical payload hash across all 5 runs (same determinism standard as Phase 5's `run_demo.py`). *(Superseded 2026-09-08 by the full sweep below — free selection made "repeat four fixed cases" the wrong shape of test. D-28.)* **Evidence log `artifacts/demo_profiles_endtoend.txt` is not committed — see D-29.**
...
- [~] **Full-sweep verification replaced the fixed-case determinism check** (D-28) — *sweep code committed; **evidence log NOT in the repo** (see below and D-29)*: every combination the schema can express, driven through the **real HTTP endpoint** on an ephemeral port (so a stale server on 8765 cannot mask it). **648 combinations swept: 81 scored (3 SOW / 78 WAIT), 243 no-decision (already sown), 324 refused with a clean validation message, 0 failures, 0 tracebacks reaching the client.** The 3-SOW/78-WAIT split independently reproduces D-27's grid dump. Each preset was also confirmed byte-identical to its free-form equivalent — proof there is one decision path, not two.
  - [ ] 🔴 **Re-run the sweep and commit its log.** `artifacts/demo_profiles_endtoend.txt` is cited by D-27, D-28 and the counts above, but it is **absent from the working tree and from every commit** — `.gitignore`'s `/artifacts/*` allowlist had no exception for it, so it was silently ignored on the machine that produced it (allowlist fixed 2026-09-08; see **D-29**). The sweep *code* (`src/demo/run_profiles_demo.py`) is committed and intact. **Until the log is regenerated by a real run, the 648/81/243/324 counts above are unverified in this repo** — they are reproducible in principle, not evidenced in fact.
  - [ ] 🔴 **Attempted 2026-09-08, still blocked — see D-30.** Python 3.13 is available in this environment (D-29's assumption that 3.11.15 was still the blocker was checked and found stale) and the pinned `requirements.txt` installs clean under it. The re-run stopped one step earlier than expected: `src/demo/case.py::build_case()` needs real IMD gridded rainfall (`imdlib` → imdpune.gov.in) and real teleconnection indices (NOAA CPC / NOAA PSL / BOM) to build the Stage 1 prior the sweep runs against, and this environment's outbound network policy denies all four hosts (verified per-host, not assumed — see D-30). No cached copy of either exists anywhere in this environment or in any branch. **Not worked around** with synthetic data or an AI-summarized fetch of the same numbers — either would be exactly the fabrication this project exists to prevent.

---

## Ongoing / cross-cutting

- [x] Correct **"120 million"** → **93.09 million agricultural households** everywhere it appears — *verified 2026-09-08 by grep across the whole tree: the corrected 93.09M figure is carried in `RESEARCH.md:51`, `DATA.md:62` and `DISCUSSION.md:31`, and each of the three surviving "120 million" mentions (`RESEARCH.md:63`, `DATA.md:66`, `DISCUSSION.md:31`) is an explicit **debunking** of it, not a use of it. **No source file or script uses 120M as a live figure.***
  - [ ] Scope limit: **slides are out of reach of this check** — no `.pptx` or any deck file exists in this repo or in any commit (verified 2026-09-08). If a deck exists off-repo, the figure must be checked there separately.
- [ ] Keep `RESEARCH.md` updated as new claims enter *(root, not `docs/` — D-10)*
- [ ] Log every architectural decision in `DISCUSSION.md` *(root, not `docs/` — D-10)*
